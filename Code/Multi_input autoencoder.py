...
filepathtrain = 'C://Users//anibo//Downloads//AIRToolsII-master//trainingdata.mat'
filepathtest = 'C://Users//anibo//Downloads//AIRToolsII-master//testdata.mat'
...
for k, v in f.items():
    arraystrain[k] = np.array(v)
x_train = np.array(arraystrain['trainingdata'])
x_train_noisy = np.array(arraystrain['trainingdata_noisy'])

f = hp.File(filepathtest, 'r')
arraystest = {}
for k, v in f.items():
    arraystest[k] = np.array(v)
x_test = np.array(arraystest['testingdata'])
x_test_noisy = np.array(arraystest['testingdata_noisy'])
